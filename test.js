...
test('Expect all cases for iteration 1 to include at least 1 number and 1 letter, minimum 8 characters long.', () => {
  expect(allIterations.iteration1('hello123')).toMatch('The password is accepted');
  expect(allIterations.iteration1('hello12')).toMatch('The password is not accepted');
  expect(allIterations.iteration1('helloafdads')).toMatch('The password is not accepted');
  expect(allIterations.iteration1('123123123')).toMatch('The password is not accepted');
})
